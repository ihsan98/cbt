...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_video`  AS SELECT `v`.`id_video` AS `id_video`, `m`.`id_materi` AS `id_materi`, `m`.`kode_materi` AS `kode_materi`, `m`.`nama_materi` AS `nama_materi`, `m`.`guru` AS `guru`, `m`.`mapel` AS `mapel`, `m`.`kelas` AS `kelas`, `m`.`text_materi` AS `text_materi`, `m`.`date_created` AS `date_created`, `v`.`judul_video` AS `judul_video`, `v`.`kode_video` AS `kode_video`, `v`.`deskripsi_video` AS `deskripsi_video`, `v`.`waktu_kuis` AS `waktu_kuis`, `v`.`terakhir_diperbarui` AS `terakhir_diperbarui` FROM (`video` `v` join `materi` `m`) WHERE `v`.`id_materi` = `m`.`id_materi` ;
